...
import com.cnpc.promoretail.ruleengine.model.RuleBenefit;
...
class RuleEngineInitialTest {
    ...
    void fixedPriceUsesRealNinePointNineAreaSample() {
        OrderContext context = order(List.of(item("70424725", "奥利奥 0糖夹心饼干 97g", 1, "10.00", "零食", "20")));
        PromotionRule rule = rule("fixed-9_9", PromotionRuleType.FIXED_PRICE,
                new RuleCondition(Set.of("70424725"), Set.of(), Set.of(), Set.of(), Set.of(), null, null,
                        BigDecimal.ZERO, BigDecimal.ZERO, false, new BigDecimal("1")),
                RuleBenefit.fixedPrice(new BigDecimal("9.90")),
                "direct_discount");

        CalculationResult result = engine.calculate(context, List.of(rule));

        assertThat(result.payableAmount()).isEqualByComparingTo("9.90");
        assertThat(result.discountAmount()).isEqualByComparingTo("0.10");
    ...
    void dayNineFullStoreDiscountExcludesCigarettes() {
        ...
        PromotionRule rule = rule("day-9-discount", PromotionRuleType.PERCENTAGE_DISCOUNT,
                new RuleCondition(Set.of(), Set.of("香烟", "化肥"), Set.of(), Set.of("gas_station"),
                        Set.of(9, 19, 29), null, null, BigDecimal.ZERO, BigDecimal.ZERO, false, BigDecimal.ZERO),
                RuleBenefit.percentageDiscount(new BigDecimal("0.90")),
                "direct_discount");
    ...
    void cigaretteOnlyCartReturnsBlockedReasonForFullStoreDiscount() {
        ...
        assertThat(result.blockedPromotions().getFirst().reasons()).anySatisfy(text -> assertThat(text).contains("排除品类"));
    }
    ...
    void gasolineAmountCanTriggerExchangePurchase() {
        ...
        PromotionRule rule = rule("gasoline-water-exchange", PromotionRuleType.EXCHANGE_PURCHASE,
                new RuleCondition(Set.of("70545526"), Set.of(), Set.of(FuelType.GASOLINE, FuelType.DIESEL),
                        Set.of(), Set.of(), null, null, BigDecimal.ZERO, new BigDecimal("180.00"),
                        false, new BigDecimal("1")),
                RuleBenefit.exchangePurchase(new BigDecimal("2.00")),
                "exchange_purchase");
    ...
    void insufficientFuelAmountBlocksExchangePurchase() {
        ...
        PromotionRule rule = rule("gasoline-water-exchange", PromotionRuleType.EXCHANGE_PURCHASE,
                new RuleCondition(Set.of("70545526"), Set.of(), Set.of(FuelType.GASOLINE),
                        Set.of(), Set.of(), null, null, BigDecimal.ZERO, new BigDecimal("180.00"),
                        false, new BigDecimal("1")),
                RuleBenefit.exchangePurchase(new BigDecimal("2.00")),
                "exchange_purchase");
        ...
        assertThat(result.recommendedCandidateId()).isEqualTo("original-price");
        assertThat(result.blockedPromotions().getFirst().reasons()).anySatisfy(text -> assertThat(text).contains("油品消费金额"));
    }
    ...
    void amountOffRuleCalculatesPayableAmount() {
        ...
        PromotionRule rule = rule("mid-autumn-amount-off", PromotionRuleType.AMOUNT_OFF,
                new RuleCondition(Set.of("moon-cake"), Set.of(), Set.of(), Set.of(), Set.of(), null, null,
                        new BigDecimal("326.00"), BigDecimal.ZERO, false, BigDecimal.ZERO),
                RuleBenefit.amountOff(new BigDecimal("66.00")),
                "direct_discount");
    ...
    void giftItemRuleReturnsGift() {
        ...
        PromotionRule rule = rule("buy-gift", PromotionRuleType.GIFT_ITEM,
                new RuleCondition(Set.of("milk"), Set.of(), Set.of(), Set.of(), Set.of(), null, null,
                        new BigDecimal("40.00"), BigDecimal.ZERO, false, BigDecimal.ZERO),
                RuleBenefit.giftItem("gift-milk", "自有奶", 1),
                "gift");

        CalculationResult result = engine.calculate(context, List.of(rule));

        assertThat(result.availableCandidates().getFirst().gifts()).hasSize(1);
    ...
    void giftCouponRuleReturnsCoupon() {
        ...
        PromotionRule rule = rule("fuel-gift-coupon", PromotionRuleType.GIFT_COUPON,
                new RuleCondition(Set.of(), Set.of("香烟", "化肥"), Set.of(FuelType.GASOLINE), Set.of(), Set.of(),
                        null, null, BigDecimal.ZERO, new BigDecimal("230.00"), true, BigDecimal.ZERO),
                RuleBenefit.giftCoupon("6元便利店商品券", new BigDecimal("6.00")),
                "coupon");

        CalculationResult result = engine.calculate(context, List.of(rule));

        assertThat(result.availableCandidates().getFirst().coupons()).hasSize(1);
        assertThat(result.availableCandidates().getFirst().coupons().getFirst().couponName()).contains("便利店");
    }
    ...
    void bundlePriceRuleCalculatesPackageDiscount() {
        ...
        PromotionRule rule = rule("bundle-driving-package", PromotionRuleType.BUNDLE_PRICE,
                new RuleCondition(Set.of("bundle-a", "bundle-b"), Set.of(), Set.of(), Set.of(), Set.of(), null, null,
                        BigDecimal.ZERO, BigDecimal.ZERO, false, BigDecimal.ZERO),
                RuleBenefit.bundlePrice(new BigDecimal("25.00")),
                "bundle");
    ...
    void inventoryShortageBlocksPromotion() {
        ...
        assertThat(result.recommendedCandidateId()).isEqualTo("original-price");
        assertThat(result.blockedPromotions().getFirst().reasons()).anySatisfy(text -> assertThat(text).contains("库存不足"));
    }
    ...
    private static PromotionRule rule(
        ...
        return new PromotionRule(ruleId, ruleId, type, 50, exclusiveGroup, false,
                PromotionRuleStatus.ACTIVE, condition, benefit, "test-v1");
    }
